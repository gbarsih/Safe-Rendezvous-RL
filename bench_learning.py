...
with open('log_perf.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(["Location", "activation", "n_l", "n_n", "p_d", "MSE", "MAE", "Loss STD"])
...
for leg in res:
    ...
    fields = [utils.getCityName(city), act, n_l, n_n, d_p, mmse, mmae, loss_std]
    print(fields)
    with open('log_perf.csv', 'a') as f:
        writer = csv.writer(f)
